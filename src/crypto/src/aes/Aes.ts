 ...
 * 
 * https://github.com/chrisveness/crypto/blob/master/aes.js
 */
export class Aes{

    // sBox is pre-computed multiplicative inverse in GF(2^8) used in subBytes and keyExpansion [§5.1.1]
    private static _sBox = [
     ...
     */
    private static _cipher(input:number[], w:number[][]) {
        ...
        let state:Array<any> = [ [], [], [], [] ];  // initialise 4×Nb byte-array 'state' with input [§3.4]
        for (let i=0; i<4*Nb; i++) state[i%4][Math.floor(i/4)] = input[i];

        state = Aes._addRoundKey(state, w, 0, Nb);

        for (let round=1; round<Nr; round++) {
            state = Aes._subBytes(state, Nb);
            state = Aes._shiftRows(state, Nb);
            state = Aes._mixColumns(state, Nb);
            state = Aes._addRoundKey(state, w, round, Nb);
        }

        state = Aes._subBytes(state, Nb);
        state = Aes._shiftRows(state, Nb);
        state = Aes._addRoundKey(state, w, Nr, Nb);
     ...
     */
    private static _keyExpansion(key:string):number[][] {
        const Nb:number = 4;            // block size (in words): no of columns in state (fixed at 4 for AES)
        const Nk:number = key.length/4; // key length (in words): 4/6/8 for 128/192/256-bit keys
        const Nr:number = Nk + 6;       // no of rounds: 10/12/14 for 128/192/256-bit keys
        ...
        for (let i=Nk; i<(Nb*(Nr+1)); i++) {
            ...
            if (i % Nk == 0) {
                temp = Aes._subWord(Aes._rotWord(temp));
                for (let t=0; t<4; t++) temp[t] ^= Aes._rCon[i/Nk][t];
            }
            // 256-bit key has subWord applied every 4th word
            else if (Nk > 6 && i%Nk == 4) {
                temp = Aes._subWord(temp);
            }
     ...
     * @private
     */
    private static _subBytes(s:any, Nb:number) {
        for (let r=0; r<4; r++) {
            for (let c=0; c<Nb; c++) s[r][c] = Aes._sBox[s[r][c]];
        }
        return s;
    }
     ...
     * @private
     */
    private static _shiftRows(s:any, Nb:number) {
        const t = new Array(4);
        for (let r=1; r<4; r++) {
     ...
     * @private
     */
    private static _mixColumns(s:any, Nb:number) {
        for (let c=0; c<Nb; c++) {
     ...
     * @private
     */
    private static _addRoundKey(state:any, w:any, rnd:number, Nb:number) {
        for (let r=0; r<4; r++) {
     ...
     * @private
     */
    private static _subWord(w:any) {
        for (let i=0; i<4; i++) w[i] = Aes._sBox[w[i]];
        return w;
    }
    ...
    private static _getBits(nBits:number):number{
        return Tools._InArray(nBits.toString(),['128','192','256']) ? nBits : 256;
    }
     ...
     */
    private static _encrypt(plaintext:string, password:string, nBits:number=256) {
        nBits = Aes._getBits(nBits);
        plaintext = Aes._utf8Encode(String(plaintext));
        password = Aes._utf8Encode(String(password));
        ...
        const pwBytes:any = new Array(nBytes);
        for (let i=0; i<nBytes; i++) { // use 1st 16/24/32 chars of password for key
            pwBytes[i] = i<password.length ?  password.charCodeAt(i) : 0;
        }
        let key:any = Aes._cipher(pwBytes, Aes._keyExpansion(pwBytes)); // gives us 16-byte key
        ...
        // ------------ perform encryption ------------
        const ciphertextBytes = Aes._nistEncryption(plaintextBytes, key, counterBlock);
        ...
        const ciphertextB64 =  Aes._base64Encode(nonceStr+ciphertextUtf8);

        return ciphertextB64;
    }


    public static e(plaintext:string, password:string, nBits:number=256):string{
        return Aes._encrypt(plaintext,password,nBits);
    }
     ...
     * @private
     */
    private static _nistEncryption(plaintext:string|any, key:string, counterBlock:any) {
        const blockSize:number = 16; // block size fixed at 16 bytes / 128 bits (Nb=4) for AES

        // generate key schedule - an expansion of the key into distinct Key Rounds for each round
        const keySchedule = Aes._keyExpansion(key);
        ...
        for (let b=0; b<blockCount; b++) {
            ...
            const cipherCntr = Aes._cipher(counterBlock, keySchedule);
     ...
     */
    private static _decrypt(ciphertext:string, password:string, nBits:number=256) {
        nBits = Aes._getBits(nBits);
        ciphertext = Aes._base64Decode(String(ciphertext));
        password = Aes._utf8Encode(String(password));
        ...
        let key:any = Aes._cipher(pwBytes, Aes._keyExpansion(pwBytes));
        key = key.concat(key.slice(0, nBytes-16)); // expand key to 16/24/32 bytes long
        ...
        // ------------ perform decryption ------------
        const plaintextBytes = Aes._nistDecryption(ciphertextBytes, key, counterBlock);
        ...
        const plaintext = Aes._utf8Decode(plaintextUtf8);

        return plaintext;
    }

    public static d(ciphertext:string, password:string, nBits:number=256):string{
        return Aes._decrypt(ciphertext,password,nBits);
    }
     ...
     * @private
     */
    private static _nistDecryption(ciphertext:string|any, key:string, counterBlock:any) {
     ...
     */
    private static _utf8Encode(str:string):string{
     ...
     */
    private static _utf8Decode(str:string):string{
